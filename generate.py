...
# ---------------------------------------------------------------- runner
TRIVIAQA_URL = ("https://huggingface.co/api/datasets/mandarjoshi/trivia_qa/"
                "parquet/rc.nocontext/validation/0.parquet")

def load_questions(n, seed=0):
    path = os.path.join(HERE, "data", "triviaqa_val.parquet")
    if not os.path.exists(path):          # public benchmark, fetched on first run
        os.makedirs(os.path.dirname(path), exist_ok=True)
        print("  downloading TriviaQA (public benchmark, ~7MB) ...")
        r = requests.get(TRIVIAQA_URL, timeout=600); r.raise_for_status()
